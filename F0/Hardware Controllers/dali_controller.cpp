...
void DaliController::InitTIM()
{
	...
	/*Initialize dali_tim as 1MHz (1us) Timer*/
...
{
	/**Set halfbit sampling time */
	uint16_t baud = DALI_SAMPLINGS;
	
	/**For every received pulse */
	for(int i = 0;i<recv_cnt;++i)
	{
		/**if samplings around halfbit samplings - set received buffer to PULSE_HALFBIT */
		if(recv_buf[i]>=baud-DALI_HYST && recv_buf[i]<=baud+DALI_HYST) recv_buf[i] = DALI_PULSE_HALFBIT;
		/**if samplings around full bit samplings - set received buffer to DALI_PULSE_FULLBIT */
